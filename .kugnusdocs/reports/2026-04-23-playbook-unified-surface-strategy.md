...
## 2026-04-23 Hybrid OCR Verdict

골든 샘플 `P` 기준 현재 기본 customer PPT lane은 `hybrid preview OCR`로 잠근다.

- `Test 1 - Surya`
  - `ocr_row_count = 5`
  - 슬라이드 전체 도식/흐름 복원율은 가장 높다.
  - 표/제목 계열 텍스트에는 잡음이 남는다.
- `Test 2 - Qwen`
  - `ocr_row_count = 4`
  - 표/제목 계열은 더 깔끔하다.
  - 핵심 도식 슬라이드 하나를 놓쳐 recall 이 부족하다.
- `Test 3 - Hybrid`
  - `ocr_row_count = 5`
  - slide 1~2는 `qwen`, slide 3~5는 `surya`가 선택되었다.
  - viewer 는 원본 slide preview 를 그대로 유지하면서 corpus 는 `5/5` slide OCR row 를 확보한다.

현재 acceptance:

- `PASS` single viewer 에서 원본 slide preview 가 먼저 보인다.
- `PASS` multi viewer 에서 slide 5장 전체가 세로로 이어진다.
- `PASS` corpus manifest 에 `ocr_backends = ['qwen', 'surya']`, `ocr_target_kinds = ['slide_preview']` 가 남는다.
- `PASS` visual OCR row 가 5장 모두 생성된다.
- `CURRENT GAP` Surya 가 맡은 도식 슬라이드에는 `KOM-CO`, `ком-со` 같은 OCR noise 가 남는다.

운영 기본값:

- PPT slide preview OCR: `adaptive hybrid`
- table heavy / title heavy: `qwen` 우선
- diagram / flow / process: `surya` 우선
- embedded image OCR: `qwen -> surya`
