...
class CustomerPackNativeOoxmlSmokeTests(unittest.TestCase):
    ...
    def test_messy_pptx_infers_slide_titles_and_filters_document_code(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual("native", book["origin_method"])
            self.assertEqual("not_configured", book["ocr_status"])
            self.assertEqual([], list(book.get("ocr_backends") or []))
            self.assertGreaterEqual(int(book["ocr_candidate_count"]), 1)
            self.assertEqual("slide_deck", slide_packets_payload["surface_kind"])
            self.assertEqual("slide", slide_packets_payload["source_unit_kind"])
            self.assertEqual("native", slide_packets_payload["origin_method"])
            self.assertEqual("not_configured", slide_packets_payload["ocr_status"])
            ...
            self.assertIn('customer-slide-toolbar-chrome', single_html)
            self.assertIn('class="customer-slide-toolbar-title"', single_html)
            self.assertIn("원본 슬라이드 1장", single_html)
            self.assertIn('id="slide-001"', single_html)
            self.assertNotIn('id="slide-002"', single_html)
            self.assertNotIn('class="document-footer-nav"', single_html)
            self.assertIn('class="customer-save-wiki-dock"', single_html)
            self.assertIn('class="customer-save-wiki-dock-nav"', single_html)
            self.assertIn('href="/playbooks/customer-packs/{}/index.html#slide-002"'.format(result["draft_id"]), single_html)
            self.assertIn("customer-slide-card-section", single_html)
            self.assertNotIn('class="customer-slide-card-header"', single_html)
            self.assertNotIn('class="section-header"', single_html)
            self.assertNotIn(">Quick Nav</summary>", multi_html)
            self.assertIn("카드식 슬라이드 목록", multi_html)
