...
def test_starter_questions_are_loaded_from_manifests() -> None:
    ...
    assert groups["operations"]["questions"][0]["question"] == "성능 테스트 결과를 받으면 목표와 조건을 어떻게 먼저 확인하면 좋을까요?"
...
def test_starter_questions_prefer_chunk_candidate_questions_when_available(monkeypatch) -> None:
    ...
    assert faq_questions[0]["question"] == "Pod가 안 뜰 때 원인을 어디서부터 좁히면 좋을까요?"
...
def test_postgres_official_faq_questions_are_actionable_korean(monkeypatch) -> None:
    ...
    assert all("어디서 확인하면 돼" not in question for question in questions)
    assert any(question.endswith("좋을까요?") for question in questions)
