...
- [x] Wiki/Library → Studio document scope 상태 전달 재검증
- [x] 공식 문서 metadata/ref 모델 설계
- [x] `document_sources.metadata` / `document_chunks.metadata`에 들어갈 learning ref schema 정의
- [x] official/study import 시 ref metadata를 채울 수 있는 최소 구현
- [x] RAG citation/answer context에서 current/next/related ref metadata를 사용할 수 있는 경계 추가
- [x] Library 문서 카드 또는 Studio scoped 상태바에 다음 학습 문서 후보 표시 구조 추가
...
- [x] "다음 단계로 이동" 버튼을 Studio scope 상태바 또는 citation preview에 추가
...
- `learning_metadata.py`가 corpus import batch의 relative path 순서와 category inference를 기준으로 document/chunk learning refs를 생성한다.
- `official-gold-import`도 official book slug 원형을 유지한 채 source/parsed/chunk metadata에 learning refs를 기록한다.
- Qdrant payload, BM25/vector RetrievalHit, Citation에 `learning` metadata를 보존하고 prompt context에는 `learning_next_refs`를 포함한다.
- Studio scoped 상태바는 active document의 `metadata.learning.next_refs`를 같은 repository 문서로 해석해 다음 학습 문서 버튼을 표시한다.
- 검증: `python -m pytest tests\test_runtime_figures_api.py tests\test_answer_context_metadata.py`, `npm --prefix apps/web run build`.
- 추가 검증: `python -m pytest tests\test_corpus_import.py tests\test_qdrant_indexer.py tests\test_answer_context_metadata.py tests\test_runtime_figures_api.py`.
- official gold 검증: `python -m pytest tests\test_official_gold_import.py tests\test_corpus_import.py tests\test_qdrant_indexer.py tests\test_answer_context_metadata.py`.
...
- 2026-05-09: 메모리와 코드 상태를 재확인했다. `Ask this document`는 `workspace.activeSourceId`, `workspace.activeDocumentId`, category 상태를 Studio로 전달하고, Studio는 scoped 상태바/문서 범위 해제/다음 학습 문서 버튼을 렌더링한다. v0.0.2 P0는 완료 상태이며 남은 미체크 항목은 v0.0.3 이후 후보 성격의 P1 확장이다.
