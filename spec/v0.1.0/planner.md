...
- [x] 채팅 답변 스트리밍 UX를 ChatGPT처럼 자연스럽게 표시
...
- [ ] OCP 배포 환경에서 app/web 재배포 후 smoke 검증 (v0.1.1 이후 배포 단계로 이월)
...
- 2026-05-11: Studio Chat 스트리밍 경로는 `answer_delta` 누적, `createThrottledMessageContentUpdater`, `requestAnimationFrame` 기반 throttled 렌더링, 빈 assistant bubble 숨김까지 반영되어 있어 완료 처리했다. OCP 배포 smoke는 dev merge/GHCR publish 이후 실제 `pbs-ocpops` namespace에서 확인해야 하므로 아직 남겨둔다.
- 2026-05-11: 사용자 결정에 따라 OCP 배포 환경 재배포 후 smoke 검증은 v0.1.1 이후 실제 배포 단계로 이월한다.
