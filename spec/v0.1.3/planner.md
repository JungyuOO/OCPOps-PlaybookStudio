...
- [ ] backend broker ServiceAccount(`pbs-system:terminal-broker`) + ClusterRole/RoleBinding 정의
...
- [ ] `currentMode === 'live_cluster'`일 때 `sendOpsChatStream('/api/v1/chat/query/stream', ...)`로 분기
- [ ] Live 응답(`OpsChatResponse` → 기존 메시지 형) 변환 어댑터
- [ ] 최근 터미널 명령/결과를 Live chat payload에 attach (`recent_terminal_actions`)
- [ ] Live chat의 `namespace`는 자동 발급된 `pbs-user-<owner_hash[:8]>`로 라우팅
- [ ] backend `_chat_payload`가 `recent_terminal_actions`를 받아 cluster context와 함께 의도 분류/근거로 사용
