...
v0.1.4에서 만든 metadata/search signal 계약을 실제 RAG pipeline 안으로 녹인다. 별도 "Intent Agent" 서비스를 만들지 않고, 사용자 질문이 들어오면 한 번의 query understanding 단계에서 **신호 추출, 질문 정규화, embedding 검색 문장 2~3개 확장, metadata filter plan, rank signal plan**을 함께 만든 뒤 retrieval 품질을 검증한다.
...
- object/error/command family/answer shape 기반 rank boost
...
4. **rank scoring 튜닝**
   - v0.1.4 boost 값은 보수적인 초기값이다.
   - 실제 retrieval 결과를 보고 object/error/command family/answer shape/book 후보 boost를 조정한다.
...
1. **별도 Intent Agent를 만들지 않는다.**
   ...
   - 필요하면 나중에 LLM fallback을 붙일 수 있지만, v0.1.5의 기본 경로는 deterministic + rule-based pipeline이다.

2. **One-shot으로 처리한다.**
   - 신호 추출, 정규화, 오타 보정, query expansion을 여러 단계 LLM 호출로 쪼개지 않는다.
   - 한 함수 호출 결과가 retrieval plan 전체를 제공해야 한다.

3. **Agent가 filter를 만들지 않는다.**
   - 질문 이해 단계는 신호와 confidence를 만든다.
   - hard metadata filter는 코드가 policy로 만든다.
   - `book_slug`, `objects`, `commands`, `error_states`, `intent_labels`, `answer_shapes`는 hard filter가 아니라 rank signal이다.
...
금지 이유: 이 값들은 chunk마다 누락될 수 있고, 너무 강하게 걸면 정답 문서를 검색 전에 제거할 수 있다. 대신 rank scoring에서 boost/penalty로 사용한다.
...
### Step 2. one-shot builder 구현

`build_query_signal_plan(query, context)`를 추가한다.

담당:

- 기존 `understand_query_signals()` 호출
- typo/alias normalization
- `embedding_queries` 2~3개 생성
- metadata filter 생성
- rank signal projection 생성
...
- Rank scoring:
  - `rank_signals`를 `ScoreSignals`에 포함
...
1. `build_query_signal_plan()`이 한 번의 호출로 normalized query, correction notes, classification, search signals, 2~3 embedding queries, metadata filter, rank signals를 반환한다.
2. 별도 Intent Agent runtime/service/prompt 호출 없이 RAG pipeline 내부에서 동작한다.
...
1. `QuerySignalPlan` dataclass와 one-shot builder 추가
2. typo/alias normalization dictionary 추가
3. embedding query 2~3개 생성 규칙 추가
4. metadata filter/rank signal builder 분리
...
- Query understanding은 "Agent"가 아니라 "RAG pipeline 내부 one-shot signal processing"으로 부른다.

## 진행 로그

- 2026-05-15: `QuerySignalPlan` one-shot builder를 추가했다. 기존 `understand_query_signals()`를 deterministic baseline으로 사용하고, 그 위에서 alias/오타 정규화, domain-specific enrichment, 2~3개 embedding query 생성, metadata filter, rank signal projection을 한 번에 만든다.
