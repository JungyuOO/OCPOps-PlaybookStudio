...
| 항목 | Pass 기준 | Evidence |
| --- | --- | --- |
| 현재 메타 추출 상태 파악 | 어떤 코드가 어떤 메타를 만들고 어디까지 전달하는지 설명 가능 | 현재 parser, chunk metadata, Qdrant payload audit |
...
메타데이터 추출은 "안 했다"가 아니다. 기존 parser/chunk metadata/Qdrant payload 경로가 있다.
하지만 제품 기준으로는 아직 부족하고, 전용 metadata spine 구현은 현재 dev 계약에 포함하지 않는다.

정확한 표현은 다음과 같다.

- DB chunk에는 기존 metadata가 붙어 있다.
- Qdrant payload에도 일부 문서/청크 식별 메타가 전달된다.
...
전용 spine 구현은 현재 hotfix 이후 active code가 아니다. 아래 항목은 v0.1.4 corpus layer로 옮길 때의 목표 필드다.
...
- chunk 생성 시 검색/필터/인용에 필요한 최소 metadata를 보존한다.
- 결과는 기존 chunk metadata와 향후 corpus layer metadata로 분리한다.

### 기존 chunk backfill 기준

- 기존 `document_chunks`를 직접 덮어쓰기 전에 dry-run mapping과 검증 SQL을 먼저 둔다.
- backfill은 별도 합의 후 migration/adapter 계약과 함께 진행한다.
...
- scope별 문서 수, chunk 수, metadata coverage, golden question 후보, known blocker를 만든다.
- 구현은 v0.1.4 MVP migration/adapter 범위가 잠긴 뒤 별도 작업으로 둔다.
