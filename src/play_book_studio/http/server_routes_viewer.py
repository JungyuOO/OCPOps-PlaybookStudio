...
def _viewer_source_meta(root_dir: Path, viewer_path: str) -> dict[str, Any] | None:
    ...
    if figure_parsed is not None:
        ...
        truth = official_runtime_truth_payload(settings=settings, manifest_entry=manifest_entry)
        figure_source_lane = str(manifest_entry.get("source_lane") or truth.get("source_lane") or "").strip()
        figure_approval_state = str(
            manifest_entry.get("approval_state")
            or manifest_entry.get("approval_status")
            or truth.get("approval_state")
            or ""
        ).strip()
        figure_publication_state = str(
            manifest_entry.get("publication_state") or truth.get("publication_state") or "active"
        ).strip()
        ...
        return {
            ...
            "viewer_path": viewer_path,
            "section_match_exact": True,
            "source_lane": figure_source_lane,
            "approval_state": figure_approval_state,
            "publication_state": figure_publication_state,
