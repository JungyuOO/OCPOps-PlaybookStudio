...
def _scenario_key(slide: dict[str, Any]) -> tuple[str, str]:
    ...
    return f"slide-{int(slide.get('slide_no') or 0):03d}", title or f"Integration Slide {int(slide.get('slide_no') or 0)}"
...
def parse_integration_test_deck(pptx_path: Path, slide_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for slide in slide_rows:
        ...
        scenario_no, scenario_title = _scenario_key(slide)
        if scenario_no.startswith("slide-"):
            scenario_no = document_front_matter_id(pptx_path)
            scenario_title = scenario_title or "문서 앞부분"
        zones, relations = derive_slide_semantics(slide)
        parent_chunk_id = f"ITG-{deck_key}-{scenario_no}"
        parent = grouped.setdefault(
            scenario_no,
            {
                ...
                "title": scenario_title or f"Integration Scenario {scenario_no}",
                "native_id": scenario_no,
                "variant": None,
                "chunk_kind": "integration_scenario_summary",
                "parent_chunk_id": None,
                "child_chunk_ids": [],
                "body_parts": [],
                "structured": {"scenario_id": scenario_no},
                "slide_refs": [],
                "image_attachments": [],
                "visual_summary": None,
                "visual_text": "",
        ...
        has_detail_keyword = any(keyword in title_text for keyword in ("상세", "결과", "실패", "롤백"))
        if (has_images or has_table_like) and has_detail_keyword and _has_detail_signal(body_text, attachments, zones):
            ...
            child = {
                ...
                "stage_id": "integration_test",
                "title": str(slide.get("title") or f"{scenario_title} detail"),
                "native_id": scenario_no,
                "variant": None,
                "chunk_kind": "integration_slide_detail",
                "parent_chunk_id": parent_chunk_id,
                "child_chunk_ids": [],
                "body_md": body_text,
                "structured": {"scenario_id": scenario_no, "slide_no": slide_no},
            ...
            grouped.setdefault("__children__", {}).setdefault("items", []).append(finalize_chunk(child, native_id=scenario_no))
    ...
    return {
        "deck": deck_metadata(pptx_path=pptx_path, template_family="integration_test", slide_rows=slide_rows, chunk_count=len(chunks)),
        "chunks": chunks,
    }


def parse_integration_test_graph(pptx_path: Path, slide_graph: dict[str, Any]) -> dict[str, Any]:
    ...
    for slide in graph_rows:
        ...
        parent = grouped.setdefault(
            scenario_no,
            {
                ...
                "child_chunk_ids": [],
                "body_parts": [],
                "structured": {"scenario_id": scenario_no, "layout_type": str(slide.get("layout_type") or "mixed")},
                "slide_refs": [],
                "image_attachments": [],
                "visual_summary": None,
                "visual_text": "",
                ...
                "zone_relations": [],
                "related_official_docs": [],
                "facets": {"scenario_ids": [scenario_no]},
                "source_pptx": str(pptx_path),
                "source_slide_range": [slide_no, slide_no],
            },
        )
        ...
        if (has_images or has_table_like) and has_detail_keyword and _has_detail_signal(body_text, attachments, zones):
            child = {
                ...
                "body_md": body_text,
                "structured": {"scenario_id": scenario_no, "slide_no": slide_no},
                "facets": {"scenario_ids": [scenario_no], "slide_nos": [slide_no]},
                "slide_refs": [parent["slide_refs"][-1]],
                "image_attachments": attachments,
                "visual_summary": None,
                "visual_text": "",
    ...
    chunks = sorted(parents, key=lambda item: str(item.get("chunk_id") or "")) + children
    return {
        "deck": deck_metadata(pptx_path=pptx_path, template_family="integration_test", slide_rows=graph_rows, chunk_count=len(chunks)),
        "chunks": chunks,
    }


__all__ = ["parse_integration_test_deck", "parse_integration_test_graph"]
