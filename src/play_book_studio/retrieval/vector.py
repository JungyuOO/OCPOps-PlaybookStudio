...
def hit_from_payload(payload: dict[str, Any], *, source: str, score: float) -> RetrievalHit:
    return RetrievalHit(
        ...
        metadata_confidence=str(payload.get("metadata_confidence", "")),
        answerable_questions=tuple(str(item) for item in (payload.get("answerable_questions") or []) if str(item).strip()),
