...
def understand_query(query: str) -> QueryUnderstanding:
    ...
    openshift = bool(_OPENSHIFT_RE.search(text))
    install = bool(_INSTALL_RE.search(text))
    command = bool(_COMMAND_RE.search(text))
    troubleshooting = bool(_TROUBLE_RE.search(text))
    secret_config = bool(_SECRET_CONFIG_RE.search(text))
    namespace = bool(_NAMESPACE_RE.search(text))
